...
export function Portfolio(){
 ...
 return <section id="work" className="portfolio section-pad"><div className="shell"><SectionLabel index="04">Selected work</SectionLabel><div className="portfolio-head"><h2>Work that<br/><em>stays with you.</em></h2><div><p>A selection of identities and objects built to earn attention, not demand it.</p><div className="filters" role="group" aria-label="Filter projects">{["All","Branding","Packaging"].map(x=><button key={x} className={filter===x?"active":""} onClick={()=>setFilter(x)}>{x}</button>)}</div></div></div><div className="project-grid">{shown.map((p,i)=><Reveal key={p.name} className={`project-wrap ${p.shape}`} delay={(i%3)*80}><button className="project-card" onClick={()=>setSelected(p)} aria-label={`View ${p.name} project`}><div className="project-image"><img src={p.img} alt={`${p.name} — ${p.desc}`} loading={i>0?"lazy":"eager"}/><span className="view-project">View <ArrowUpRight/></span></div><div className="project-meta"><span>0{i+1}</span><div><h3>{p.name}</h3><p>{p.desc}</p></div><small>{p.cat} · {p.year}</small><ArrowUpRight/></div></button></Reveal>)}</div></div>{selected&&<div className="project-modal" role="dialog" aria-modal="true" aria-label={`${selected.name} project`} onMouseDown={e=>{if(e.target===e.currentTarget)setSelected(null)}}><Button variant="iconDark" size="icon" onClick={()=>setSelected(null)} aria-label="Close project"><X/></Button><div className="modal-inner"><img src={selected.img} alt={`${selected.name} project presentation`}/><div><p>{selected.cat} · {selected.year}</p><h2>{selected.name}</h2><span>{selected.desc}</span></div></div></div>}</section>
}
